...
{
    public class TipoFirmanteService : ITipoFirmanteService
    {
        private readonly ITipoFirmanteRepository _repository;
        private readonly IMensajeRespuestaRepository _mensajeRepository;

        public TipoFirmanteService(ITipoFirmanteRepository repository, IMensajeRespuestaRepository mensajeRepository)
        {
            _repository = repository;
            _mensajeRepository = mensajeRepository;
        }

        public async Task<Respuesta> ObtenerTipoFirmantes()
        {
            ERespuesta codigoRespuesta = ERespuesta.OK;
            int codigoEvento = 0;

            (int codigo, IEnumerable<ComboDto> TipoFirmantes) = await _repository.SelectTipoFirmantes();
