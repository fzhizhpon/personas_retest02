...
namespace Catalogo.Application.Services
{
    public class MonedaService : IMonedaService
    {
        ...
        public MonedaService(IMonedaRepository repository, IMensajeRespuestaRepository mensajeRepository)
        {
            _repository = repository;
            _mensajeRepository = mensajeRepository;
        }

        public async Task<Respuesta> ObtenerMonedas()
        {
            ERespuesta codigoRespuesta = ERespuesta.OK;
            int codigoEvento = 0;

            (int codigo, IEnumerable<ComboDto> monedas) = await _repository.SelectMonedas();
